...
func validateConfig() (bool, bool, string, float64, float64, []float64, float64, float64) {
	...
	if err != nil {
		...
		log.Trace().Msg("Configuration file opened successfully")
	}
	...
	if err != nil {
		...
		log.Trace().Msg("Configuration data unmarshaled successfully")
	}
	...
	log.Trace().Float64("payload.Round_to_minutes", payload.Round_to_minutes).Send()
	log.Trace().Floats64("payload.Round_to_minutes_list", payload.Round_to_minutes_list).Send()
	log.Trace().Bool("using_list", using_list).Send()
	...
	log.Trace().Float64("payload.Round_up_minutes", payload.Round_up_minutes).Send()
	log.Trace().Float64("payload.Round_up_percent", payload.Round_up_percent).Send()
	log.Trace().Bool("using_percent", using_percent).Send()
...
func calculateAndDisplayJime(t time.Time, clear_screen bool, log_level string, round_to_minutes float64, round_to_minutes_list []float64, round_up_minutes float64, round_up_percent float64) {
	now_minute := t.Minute()
	log.Trace().Int("now_minute", now_minute).Send()
	...
	if using_list {
		...
		log.Debug().Float64("low_round_to_minute", low_round_to_minute).Send()
		log.Debug().Float64("high_round_to_minute", high_round_to_minute).Send()
	...
	log.Debug().Dur("round_to_duration, seconds", round_to_duration).Send()
	...
	log.Debug().Dur("round_up_duration, seconds", round_up_duration).Send()
...
func main() {
	...
	log.Trace().Float64("loop_seconds", loop_seconds).Send()
	log.Trace().Float64("round_up_minutes", round_up_minutes).Send()
	...
	if loop_duration != 0 {
		for {
			...
			log.Trace().Time("next_loop_time", next_loop_time).Send()
			sleep := next_loop_time.Sub(t)

			if sleep < 0 {
				sleep = next_loop_time.Sub(t) + loop_duration
			}
			log.Trace().Dur("sleep, seconds", sleep).Send()
